...
> ⚠️ **架构环境差异补充（2.14.11）**：pnpm 按系统原生 arch 选装 rollup 可选包，但 NVM x64 Node.js 在 arm64 机器上运行时会导致测试阻塞。optionalDependencies 策略已更新至同时覆盖 darwin-arm64 与 darwin-x64。项目放行标准：以主开发机可复核结果（arm64 原生 Node.js + pnpm）为准，CI 结果为辅。
